# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — South Korea

## Classification

- Repository: `cloud-itonami-iso3166-kor`
- ISO 3166: `KOR` (South Korea)
...
- Social impact: [:sme-purchase-promotion-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Korean
  public contract
...
- registration walkthrough for KONEPS (Korea ON-line E-Procurement System), the Public Procurement Service's unified national e-procurement platform
- business/tax registration checklist: business registration (사업자등록증) with the National Tax Service; VAT registration with the National Tax Service
- local-content / preferential-procurement navigation: the SME product purchase-promotion program (중소기업제품 구매촉진 및 판로지원에 관한 법률), which reserves a share of public purchasing for registered small and medium enterprises
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Korea-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in South Korea).
