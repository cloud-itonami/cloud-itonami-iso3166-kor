(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a South Korea-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. South Korea-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-kr-presence?
                                       true` (near-universal for South Korea
                                       ...
                                       Grounded in Korean presence or
                                       authorized local representative for many
                                       public tenders (PPS / contracting
                                       authorities).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(BRN/統一資格/KONEPS登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- kr-presence-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-kr-presence? true`, INDEPENDENTLY verify
  `:has-kr-presence?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-kr-presence?` ground truth (most South Korea public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-kr-presence? e))
                 (not (true? (:has-kr-presence? e))))
        [{:rule :kr-presence-missing
          :detail (str subject " は韓国居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- brn-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-brn? true`, INDEPENDENTLY check
  `:brn-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-brn? e))
                 (not (true? (:brn-verified? e))))
        [{:rule :brn-unverified
          :detail (str subject " はBRN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (kr-presence-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (brn-unverified-violations request st)
