...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (KOR, clean) ==")
    ...
    (println "== jurisdiction/assess eng-4 (sets up kr-presence-missing) ==")
    ...
    (println "== filing/submit eng-4 (kr-presence-missing -> HARD hold) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (sets up brn-unverified) ==")
    ...
    (println "== filing/submit eng-5 (brn-unverified -> HARD hold) ==")
