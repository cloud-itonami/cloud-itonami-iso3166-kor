(ns marketentry.marketentryllm
  ...
  the KOR public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-kr-presence?=" (:has-kr-presence? e)
                        " brn-verified?=" (:brn-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-kr-presence? e))
                              (:has-kr-presence? e))
                          (or (not (:requires-brn? e))
                              (:brn-verified? e)))
                   0.9 0.3)}))
